...
TEMPERATURE = 4.0
ALPHA = 0.7
INITIAL_STUDENT_LR = 0.001 # Initial LR for student, can be tuned
...
def get_data_loaders(data_dir, batch_size, num_workers, pin_memory, num_classes_from_data=False):
    ...
    try:
        ...
        test_loader_inf = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=num_workers, pin_memory=pin_memory)
...
if __name__ == '__main__':
    ...
    # --- Student Model ---
    print(f"\nInitializing Student Model ({STUDENT_ARCHITECTURE})...")
    student_model = get_model(STUDENT_ARCHITECTURE, NUM_CLASSES, pretrained=False)
    optimizer = optim.Adam(student_model.parameters(), lr=INITIAL_STUDENT_LR) # Adam is common for KD
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} - Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f} | Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f} | Time: {train_duration+val_duration:.2f}s")
